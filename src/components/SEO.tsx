...
export default function SEO({ title, description, canonical, ogImage, schema }: SEOProps) {
  ...
  // Generate a per-page OG image with the page title baked in.
  // Falls back to the static site-level image if no title.
  const ogImg = ogImage || `https://www.makeitglow.co/api/og/italian-film-salon?title=${encodeURIComponent(title)}&subtitle=${encodeURIComponent(description.slice(0, 80))}`;
